...
RUN --mount=type=cache,dst=/root/.cache/ echo general-clean && rm -rf /opt/conda && rm -rf /var/lib/apt/lists/* && apt clean \
...
COPY .uv/ ./
RUN --mount=type=cache,dst=/root/.cache/ uv python install --preview --default

COPY pyproject.toml uv.lock ./
RUN --mount=type=cache,dst=/root/.cache/ uv sync
